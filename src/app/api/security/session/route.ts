...
export async function POST(req:NextRequest){
  ...
  // RLS deliberately hides suspended profiles. Read only the verified caller's
  // status server-side so suspension is not confused with an unprovisioned user.
  const {data:profile,error}=await supabaseAdmin().from("profiles").select("account_type,account_status").eq("id",user.id).maybeSingle();
