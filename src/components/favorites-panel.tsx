...
export function FavoritesPanel({locale="en"}:{locale?:string}){
 ...
 async function load(){const r=await fetch("/api/favorites");if(r.ok)setItems(await r.json());else setMessage(ar?"المفضلة متاحة لحسابات العملاء فقط.":"Favorites are available to client accounts.")}
 useEffect(()=>{void load()},[]);
 async function remove(id:string){const r=await fetch("/api/favorites?talentId="+encodeURIComponent(id),{method:"DELETE"});if(r.ok)await load()}
