...
export function VerificationDocuments({locale="en"}:{locale?:string}){
  ...
  async function load(){const r=await apiFetch("/api/verification/documents");if(r.ok)setItems(await r.json())}
  useEffect(()=>{void load()},[]);
  async function upload(file:File){
    setBusy(true);setMessage(ar?"جارٍ رفع المستند…":"Uploading document…");
    const prep=await apiFetch("/api/verification/documents",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({fileName:file.name,mimeType:file.type,sizeBytes:file.size})});
    ...
    const save=await apiFetch("/api/verification/documents",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:p.path,mimeType:file.type,label:label||file.name})});
  ...
  async function remove(id:string){if(!confirm(ar?"حذف هذا المستند؟":"Delete this document?"))return;await apiFetch("/api/verification/documents?id="+encodeURIComponent(id),{method:"DELETE"});await load()}
