...
export function TeamMembers({locale="en"}:{locale?:string}){
  ...
  async function add(e:FormEvent<HTMLFormElement>){
    ...
    if(r.ok){formEl.reset();setImagePath(null);setImagePreview(null);await load()}setBusy(false);
  }
  async function remove(id:string){if(!confirm(ar?"إزالة هذا العضو من ملف الفريق؟":"Remove this team member?"))return;await apiFetch("/api/team-members?id="+encodeURIComponent(id),{method:"DELETE"});await load()}
