...
export function SecurityHistory({locale="en"}:{locale?:string}){
  ...
  useEffect(()=>{void fetch("/api/security/history").then(async r=>{if(r.ok)setRows(await r.json());else setMessage(ar?"تعذّر تحميل سجل تسجيل الدخول.":"Could not load login history.")})},[ar]);
