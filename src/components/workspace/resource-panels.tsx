...
export function ProjectsPanel(){
  ...
  return <div className="grid"><p role="status">{message}</p>{items.length?items.map((x,i)=><div className="card grid" key={String(x.id??i)}><JsonCard item={x}/><div className="form-actions">{process.env.NODE_ENV==="development"&&<button className="btn" onClick={()=>fund(String(x.id))}>Simulate funding (development)</button>}<button className="btn secondary" onClick={()=>deliver(String(x.id))}>Submit delivery</button><button className="btn secondary" onClick={()=>review(String(x.id),"accept")}>Accept delivery</button><button className="btn secondary" onClick={()=>review(String(x.id),"request_revision")}>Request revision</button></div></div>):<div className="empty">No projects yet.</div>}</div>
}
