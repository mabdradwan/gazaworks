...
export function DisputesPanel({locale="en"}:{locale?:string}){
  ...
  async function load(){const [d,p]=await Promise.all([fetch("/api/disputes"),fetch("/api/projects")]);if(d.ok)setItems(await d.json());if(p.ok)setProjects(await p.json())}
  useEffect(()=>{void load()},[]);
  async function open(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/disputes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectId:f.get("projectId"),reason:f.get("reason")})});setMessage(r.ok?(ar?"تم فتح النزاع وتجميد التحويل والقبول التلقائي.":"Dispute opened; payout and automatic acceptance are frozen."):(ar?"تعذّر فتح النزاع.":"Could not open dispute."));if(r.ok){e.currentTarget.reset();await load()}}
...
function DisputeCard({dispute,locale,reload}:{dispute:Dispute;locale:string;reload:()=>Promise<void>}){
  ...
  async function loadEvidence(){const r=await fetch("/api/disputes/evidence?disputeId="+dispute.id);if(r.ok)setEvidence(await r.json())}
