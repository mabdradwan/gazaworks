...
type Project={id:string;status:string;deadline?:string|null;created_at:string;client_id:string;talent_id:string;work_request_id?:string|null;project_agreements?:Agreement|Agreement[]|null;profiles?:{display_name?:string}|null;talent?:{display_name?:string}|null};
...
export function ProjectsPanel({locale="en"}:{locale?:string}){
  ...
  async function load(){const [p,r]=await Promise.all([fetch("/api/profile"),fetch("/api/projects")]);if(p.ok)setMe(await p.json());if(r.ok)setItems(await r.json())}
  useEffect(()=>{void load()},[]);
  async function fund(id:string){if(!confirm(ar?"هذا محاكي تطوير فقط وليس دفعة حقيقية. متابعة؟":"This is a development simulator, not a real payment. Continue?"))return;const r=await fetch("/api/payments/mock-fund",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectId:id,providerFeeMinor:0})});const d=await r.json().catch(()=>({}));setMessage(r.ok?(ar?"تمت محاكاة التمويل للتطوير فقط. لا توجد عملية مالية حقيقية.":"Development payment simulated. No real charge occurred."):(d.error??(ar?"تعذّرت المحاكاة.":"Could not simulate funding.")));if(r.ok)await load()}
...
function ProjectCard({p,me,locale,onFund,onReview,reload}:{p:Project;me:Profile|null;locale:string;onFund:(id:string)=>Promise<void>;onReview:(id:string,a:"accept"|"request_revision")=>Promise<void>;reload:()=>Promise<void>}){
  ...
  async function details(){const [d,f]=await Promise.all([fetch("/api/projects/deliveries?projectId="+p.id),fetch("/api/projects/files?projectId="+p.id)]);if(d.ok)setDeliveries(await d.json());if(f.ok)setFiles(await f.json())}
  useEffect(()=>{if(expanded)void details()},[expanded]);

  async function upload(file:File,deliveryId?:string){
    const prep=await fetch("/api/projects/files",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectId:p.id,fileName:file.name,mimeType:file.type||"application/octet-stream",sizeBytes:file.size})});const x=await prep.json();if(!prep.ok)throw Error(x.error??"upload_prepare_failed");
    ...
    const done=await fetch("/api/projects/files",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectId:p.id,path:x.path,mimeType:file.type||"application/octet-stream",sizeBytes:file.size,deliveryId:deliveryId??null})});if(!done.ok)throw Error("file_record_failed");
  }
  ...
  async function deliver(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/projects/deliveries",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectId:p.id,message:f.get("message")})});const d=await r.json().catch(()=>({}));if(!r.ok){setNotice(ar?"لا يمكن التسليم قبل تأمين الدفع أو في حالة المشروع الحالية.":"Delivery requires a funded project in an eligible state.");return}try{if(deliveryFile)await upload(deliveryFile,String(d.id));setDeliveryFile(null);e.currentTarget.reset();setNotice(ar?"تم إرسال التسليم النهائي. لدى العميل 3 أيام للمراجعة.":"Final delivery submitted. The client has 3 days to review.");await details();await reload()}catch{setNotice(ar?"تم تسجيل التسليم لكن تعذّر رفع ملف التسليم.":"Delivery recorded, but its file could not be uploaded.")}}
  ...
  return <article className="card project-card">
    <div className="card-head"><div><span className={"badge project-status status-"+p.status}>{statusLabel(p.status,ar)}</span><h2>{ar?"مشروع مع":"Project with"} {party}</h2></div>{agreement&&<strong>{new Intl.NumberFormat(ar?"ar-PS":"en",{style:"currency",currency:agreement.currency}).format(agreement.price_minor/100)}</strong>}</div>
    ...
    {p.status==="funded"&&isTalent&&<div className="secure-payment-banner">✓ {ar?"تم تأمين الدفع عبر GazaWorks — يمكنك بدء العمل":"Payment secured by GazaWorks — you may begin work"}</div>}
    {p.status==="awaiting_payment"&&isClient&&<div className="card development-warning"><strong>{ar?"وضع التطوير":"Development mode"}</strong><p>{ar?"بوابة الدفع الحقيقية غير مفعلة قبل الموافقات البنكية والقانونية. المحاكي أدناه لا يجري أي خصم حقيقي.":"A real payment gateway is not enabled before banking/legal approval. The simulator below never charges real money."}</p><button className="btn" onClick={()=>void onFund(p.id)}>{ar?"محاكاة التمويل للتطوير":"Simulate funding (development only)"}</button></div>}
